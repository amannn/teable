...
export class BaseShareOpenController {
  ...
  async copyBaseShare(
    ...
    if (targetBaseId) {
      ...
      if (!targetBase) {
        throw new CustomHttpException('Target project not found', HttpErrorCode.VALIDATION_ERROR);
      }

      if (targetBase.spaceId !== spaceId) {
        throw new CustomHttpException(
          'Target project does not belong to the specified space',
  ...
  private async runShareBaseCopy(
    ...
    this.eventEmitterService.emit(Events.BASE_SHARE_COPY_COMPLETE, {
      baseId: base.id,
      fromBaseId,
      shareId,
    });
