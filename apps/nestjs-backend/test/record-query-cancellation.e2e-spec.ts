...
describe('interactive record query cancellation (real PostgreSQL / HTTP / ShareDB)', () => {
  ...
  it.skipIf(!v2)(
    'preserves the documented no-view aggregation fallback instead of claiming its V1 SQL is cancellable',
    async () => {
      const route = routes().find((route) => route.name === 'ordinary aggregation')!;
      route.query = { field: { [StatisticsFunc.Sum]: [numberId()] } };
      await successful(route);
      await evictCapturedCache();
      await lock();
  ...
  it.skipIf(!v2)(
    ...
    async () => {
      ...
      const original = service.tryGetRowCount.bind(service);
      const middleware = new V2QueryCancellationMiddleware(cls);
      const finished = deferred();
      let backgroundRows: unknown;
      let backgroundError: unknown;
      const marker = `background_${table.id}`;
      vi.spyOn(service, 'tryGetRowCount').mockImplementationOnce(async (...args) => {
