  ...
  implements ITableSpecVisitor<ReadonlyArray<TableSchemaStatementBuilder>>
{
  ...
  visitTableAddView(
    ...
    // T7541: physical `__row_<viewId>` storage is created by
    // ITableSchemaRepository.prepareViewRowOrderStorage before the request
    // transaction (ensureRowOrderColumnOnline: 3s lock_timeout ADD COLUMN,
    // chunked autocommit backfill, CREATE INDEX CONCURRENTLY). Emitting
    // ALTER/UPDATE/CREATE INDEX here holds AccessExclusiveLock through the
    // full-table backfill until that transaction commits (T7251 residual
    // risk; prod Aurora incident 2026-09-21).
    const statements: ReadonlyArray<TableSchemaStatementBuilder> = [];
    return this.addCond(statements).map(() => statements);
  }
