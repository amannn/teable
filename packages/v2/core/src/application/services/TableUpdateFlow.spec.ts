...
describe('TableUpdateFlow', () => {
  ...
  it('prepares grid view row-order storage before opening the update transaction', async () => {
    const table = buildTable();
    const order: string[] = [];
    const preparedViewIds: string[][] = [];
    const flow = new TableUpdateFlow(
      new FakeTableRepository(),
      {
        insert: async () => ok(undefined),
        insertMany: async () => ok(undefined),
        prepareViewRowOrderStorage: async (_context, _table, viewIds) => {
          order.push('prepare-row-order');
          preparedViewIds.push([...viewIds]);
          return ok(undefined);
        },
    ...
    const createdViewId = result.value.table
      .views()
      .find((view) => view.name().toString() === 'Copy')
      ?.id()
      .toString();
    expect(order).toEqual(['prepare-row-order', 'schema-update']);
    expect(preparedViewIds).toEqual([[createdViewId]]);
  });
